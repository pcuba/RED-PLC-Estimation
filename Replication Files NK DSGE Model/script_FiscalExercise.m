...
% PosteriorDraws/COPFexactZLB_Prior1_US_RClin1_GridSmolyak_Mhrun1/Filtered/
%--------------------------------------------------------------------------

%=== ADD PATH CONTAINING FUNCTIONS ===%
addpath 'FunctionsExpost/'

%=== DIRECTORIES WITH POSTERIOR DRAWS ===%
modeldir   = 'PosteriorDraws/COPFexactZLB_Prior1_US_Mhrun1/Filtered/';
modelname  = 'COPFexactZLB_Filtered_Mean_Prior1_US_Mhrun1_';

%=== DIRECTORIES TO STORE RESULTS ===%
% Keep empty to not print txt and pdf files. Use ./ for root directory
resultsdir = []; 

%=== OPTIONS FOR RESULTS ===%

period_use        = 2009.5;       % Set period for experiment on '2009Q2'
period_plot_start = 2007.75;       % Set period to start plot
ind_zero_positive_er = 1;          % If 1 zero-out possitive MP shocks for baseline                         
...
dateshocks = Year(pick_0_period:pick_initial_period + Tsim-1);
...
if ind_add_shocks == 1
    ...
    [shocks_use_inter,cases,monotone]  = solve_for_forward_guidance_old(sModel,sParam,...
            shocks_use_nointer,init_use,length_intervention_fg,...
            length_intervention_fiscal,intervention_bound);

    % GET ONLY FISCAL
    
    shocks_use_justfiscal = shocks_use_nointer;
    
    shocks_use_justfiscal(2,1:length_intervention_fiscal) = shocks_use_nointer(2,1:length_intervention_fiscal) + size_spending_intervention;
...
elseif ind_add_shocks == 0
    % The order of the shocks is: er, eg, ez, ed
...
%   shocks_use_inter(1,tWindow+1:tWindow+length_intervention_fg)     = zeros(length_intervention_fg,1);


    % Remove the contractionary shock in 2009Q3
    % shocks_er_patch2(1) = 0; %
     
    shocks_use_inter(1,tWindow+1:tWindow+length_intervention_fg)  = shocks_realized(1,tWindow+1:tWindow+length_intervention_fg);
    ...
    % Patch monetary policy shocks
    shocks_er_patch_justfiscal  = shocks_realized(1,tWindow+1:tWindow+length_intervention_fg);
    shocks_er_patch_justfiscal(shocks_er_patch_justfiscal<0) = 0; 
    shocks_use_justfiscal(1,tWindow+1:tWindow+length_intervention_fg) = ind_zero_positive_er*shocks_er_patch_justfiscal; %zeros(length_intervention_fg,1);
    ...
    % Patch monetary shocks
    shocks_er_patch_nointer  = shocks_realized(1,tWindow+1:tWindow+length_intervention_fg);
    shocks_er_patch_nointer(shocks_er_patch_nointer<0) = 0; 
    shocks_use_nointer(1,tWindow+1:tWindow+length_intervention_fg)     = shocks_er_patch_nointer; %zeros(length_intervention_fg,1);
    
    % Remove fiscal impulse
    shocks_use_nointer(2,tWindow+1:tWindow+length_intervention_fiscal) = ind_zero_positive_er*shocks_realized(2,tWindow+1:tWindow+length_intervention_fiscal) - size_spending_intervention;
    ...
    % add ARRA shock
    shocks_use_exante = shocks_use_zero;
    shocks_use_exante(2,tWindow+1:tWindow+length_intervention_fiscal) = shocks_use_justfiscal(2,tWindow+1:tWindow+length_intervention_fiscal);

    
    % SIMULATE PATHS %    
    [~,~,~,SIM_DATA]       = fSimulatePLC(sModel,sParam,init_0,shocks_realized);
    ...
    [~,~,~,SIM_EXANTE]     = fSimulatePLC(sModel,sParam,init_0,shocks_use_exante);

    
    table_er = array2table([dateshocks   shocks_realized(1,:)' shocks_use_inter(1,:)' shocks_use_justfiscal(1,:)' shocks_use_nointer(1,:)' ]);
    table_er.Properties.VariableNames={'Quarters' 'Realized','Both','Fiscal','Baseline'};
    display(table_er);


    table_eg = array2table([dateshocks   shocks_realized(2,:)' shocks_use_inter(2,:)' shocks_use_justfiscal(2,:)' shocks_use_nointer(2,:)' ]);
    table_eg.Properties.VariableNames={'Quarters' 'Realized','Both','Fiscal','Baseline'};
    display(table_eg);
...
if sum(isnan(SIM_INTER.R)) == 0    
    ...
    % PREPARE MULTIPLIERS JUST FISCAL %
    SIM_EXANTE.MU_NUM   = (SIM_EXANTE.BIGY - SIM_ZERO.BIGY);
    SIM_EXANTE.MU_DENOM = (SIM_EXANTE.BIGG - SIM_ZERO.BIGG);
    SIM_EXANTE.MU       =  SIM_EXANTE.MU_NUM ./ SIM_EXANTE.MU_DENOM;
    
    % CUMULATE EFFECTS %
    SIM_EXANTE.MU_NUM_CUM   = cumsum(SIM_EXANTE.MU_NUM);
    SIM_EXANTE.MU_DENOM_CUM = cumsum(SIM_EXANTE.MU_DENOM);
    SIM_EXANTE.MU_CUM       = SIM_EXANTE.MU_NUM_CUM ./  SIM_EXANTE.MU_DENOM_CUM;
...
% Data
plot_ep_star_R_DATA    = 400 * (SIM_DATA.R -1);
plot_ep_star_pi_DATA   = 400 * (SIM_DATA.pi -1);
plot_ep_star_Y_DATA    = SIM_DATA.BIGY;
plot_ep_star_dely_DATA = SIM_DATA.dely;
plot_ep_star_er_DATA   = SIM_DATA.er;
plot_ep_star_eg_DATA   = SIM_DATA.eg;
plot_ep_star_g_DATA    = SIM_DATA.g;
plot_ep_star_z_DATA    = SIM_DATA.z;
plot_ep_star_d_DATA    = SIM_DATA.d;
...
plot_ep_star_g_JUSTFISCAL  = SIM_JUSTFISCAL.g;
plot_ep_star_z_JUSTFISCAL    = SIM_JUSTFISCAL.z;
plot_ep_star_d_JUSTFISCAL    = SIM_JUSTFISCAL.d;
...
plot_ep_star_er_ZERO = SIM_EXANTE.er;
plot_ep_star_eg_ZERO = SIM_EXANTE.eg;
plot_ep_star_ez_ZERO = SIM_EXANTE.ez;
plot_ep_star_R_ZERO  = 400 * (SIM_EXANTE.R - 1);
plot_ep_star_pi_ZERO   = 400 * (SIM_EXANTE.pi -1);
plot_ep_star_Y_ZERO    = SIM_EXANTE.BIGY;
plot_ep_star_dely_ZERO = SIM_EXANTE.dely;
plot_ep_star_g_ZERO = SIM_EXANTE.g;
plot_ep_star_z_ZERO = SIM_EXANTE.z;
plot_ep_star_d_ZERO = SIM_EXANTE.d;
...
subplot(3,2,1)
plot(xt,plot_ep_star_eg_INTER','k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,plot_ep_star_eg_JUSTFISCAL','r-x','linewidth',2,'MarkerSize',3);hold on;
plot(xt,plot_ep_star_eg_NOINTER','b-','linewidth',2);hold on;
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
title('Gov. Spending Shock Innovations ($\epsilon_{g,t}$)','fontsize',16,'fontweight','bold','Interpreter','Latex')
axis([xt(1) xt(end) -3 5]); set(gca,'FontSize',12); box off; grid on;
...
vline(Year(pick_initial_period),'r-');


subplot(3,2,2)
plot(xt,plot_ep_star_er_INTER' ,'k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,plot_ep_star_er_JUSTFISCAL' ,'r-x','linewidth',2,'MarkerSize',3);
plot(xt,plot_ep_star_er_NOINTER' ,'b-','linewidth',2);
% plot(xt,plot_ep_star_er_DATA' ,'-o','color',[216,179,101]/256,'linewidth',2,'MarkerSize',3);
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
...
subplot(3,3,4)
plot(xt,log(plot_ep_star_g_INTER'/sParam.gstar)/sig_ghat,'k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,log(plot_ep_star_g_JUSTFISCAL'/sParam.gstar)/sig_ghat,'r-x','linewidth',2,'MarkerSize',3);hold on;
...
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
axis([xt(1) xt(end) -.1 3.5]); set(gca,'XTickLabel',mydates);
title('Gov. Spending Shock','fontsize',16,'fontweight','bold','Interpreter','Latex')
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on'; box off
vline(Year(pick_initial_period),'r-');

subplot(3,3,5)
plot(xt,log(plot_ep_star_z_INTER')/sig_zhat,'k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,log(plot_ep_star_z_JUSTFISCAL')/sig_zhat,'r-x','linewidth',2,'MarkerSize',3);hold on;
...
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
axis([xt(1) xt(end) -4 2]); set(gca,'XTickLabel',mydates);
title('Technology Growth Shock','fontsize',16,'fontweight','bold','Interpreter','Latex')
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on'; box off
vline(Year(pick_initial_period),'r-');

subplot(3,3,6)
plot(xt,log(plot_ep_star_d_INTER')/sig_dhat,'k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,log(plot_ep_star_d_JUSTFISCAL')/sig_dhat,'r-x','linewidth',2,'MarkerSize',3);hold on;
...
axis([xt(1) xt(end) -4 0]); set(gca,'FontSize',12);  box off; grid on; 
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on'; 
vline(Year(pick_initial_period),'r-');

subplot(3,3,7)
plot(xt,cumsum(plot_ep_star_dely_INTER)','k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,cumsum(plot_ep_star_dely_JUSTFISCAL)','r-x','linewidth',2,'MarkerSize',3);
...
axis([xt(1) xt(end) -6 1]); set(gca,'FontSize',12); box off; grid on;
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on';
vline(Year(pick_initial_period),'r-');

subplot(3,3,8)
plot(xt,plot_ep_star_pi_INTER','k--','linewidth',2,'MarkerSize',3);hold on;
% plot(xt,plot_ep_star_pi_JUSTFISCAL','r-x','linewidth',2,'MarkerSize',3);
% plot(xt,plot_ep_star_pi_JUSTFISCAL','r-x','linewidth',2,'MarkerSize',3);
plot(xt,plot_ep_star_pi_NOINTER','b-','linewidth',2);
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
title('Inflation (\% annualized)','fontsize',16,'fontweight','bold','Interpreter','Latex')
axis([xt(1) xt(end) -1 4]);set(gca,'FontSize',12); box off; grid on;
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on';
vline(Year(pick_initial_period),'r-');

subplot(3,3,9)
plot(xt,plot_ep_star_R_INTER','k--','linewidth',2,'MarkerSize',3);hold on;
plot(xt,plot_ep_star_R_ZERO','r:','linewidth',2,'MarkerSize',3);
plot(xt,plot_ep_star_R_NOINTER','b-','linewidth',2);
plot(xt,zeros(Tperiods,1)','k--','linewidth',1);
set(gca,'XTick',xt); set(gca,'XTickLabel',mydates);
title('Interest Rate (\% annualized)','fontsize',16,'fontweight','bold','Interpreter','Latex')
axis([xt(1) xt(end) -0.1 4]); set(gca,'FontSize',12); box off; grid on; 
ax = gca; ax.XGrid = 'off'; ax.YGrid = 'on';
vline(Year(pick_initial_period),'r-');
...
fprintf('\n----------------------------------------------------------')
fprintf('\n Period     Ex-Post (Both) Ex-Post (Just Fiscal) ')  
fprintf('\n----------------------------------------------------------')
fprintf('\n    %4.2f      %4.4f         %4.4f         ', [xt(tWindow+1:end) SIM_INTER.MU_CUM(tWindow+1:end)' SIM_JUSTFISCAL.MU_CUM(tWindow+1:end)']')
fprintf('\n----------------------------------------------------------\n')


% Difference Inflation and Output growth (annual rates)

diff_pi = plot_ep_star_pi_INTER' - plot_ep_star_pi_NOINTER';
diff_dely = plot_ep_star_dely_INTER' - plot_ep_star_dely_NOINTER';

fprintf('\n Average inflation difference a.r. (INTER - NO INTER): %4.4f \n', mean(diff_pi(8:end)));
fprintf('\n Average growth difference a.r. (INTER - NO INTER): %4.4f \n', mean(diff_dely(8:end))*4);
...
if ~isempty(resultsdir)
    print(figure(1),'-dpdf','-r400',[resultsdir 'Figure_Shocks_and_ExPost_Intervention_Final.pdf'],'-fillpage');
