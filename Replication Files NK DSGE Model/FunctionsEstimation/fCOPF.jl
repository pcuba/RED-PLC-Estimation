...
function fCOPF(mY, sParam_StateSpace, mSigmaMEtilde,pKappa, nM,pInvSigmaMEtilde)
...
for tt = 1:T
    ...
    # Propagate
    (mShock, iDlog, mS_fore, mY_fore, vRegime) = fCondDist_v5(mS_fore,mY_fore,vRegime,tt,pSigmaTE,pSigmaEta,pA,pA0,eta1BarCoeff,pPhi1_0,pPhi1_1,pPhi1_Eta,pPhi2_0,pPhi2_1,pPhi2_Eta,mSigmaMEtilde,ME_scale,mS_up,yy,nM,pInvSigmaMEtilde);
                                               # fCondDist_v5(mS_fore,mY_fore,vRegime,tt,pSigmaTE,pSigmaEta,pA,pA0,eta1BarCoeff,pPhi1_0,pPhi1_1,pPhi1_Eta,pPhi2_0,pPhi2_1,pPhi2_Eta,pSigmaMEtilde,pKappa,mS_up,yy,nM,pInvSigmaMEtilde)
    if isnan(maximum(iDlog))==1
        nFlagError = 1
        iDlog .= 1
    end
    ...
    #    (mS_fore[:,iSim], mY_fore[:,iSim], vRegime[iSim]) = fStateSpaceSimulationaug(eta1BarCoeff,Phi01,Phi11,PhiEta1,Phi02,Phi12,PhiEta2,pA,pA0, mS_up[:,iSim], mShock[:,iSim]);
         ...
         #logdensity[iSim] = logpdf(MvNormal(zeros(size(yy,1)), pKappa*mSigmaME), perror[:,iSim]) + vAdj[iSim]
         #logdensity[iSim] = ( - (ny/2)*log(2*pi) - 0.5*log(det(pKappa*mSigmaME))
          #           - 0.5*perror[:,iSim]'*inv(pKappa*mSigmaME)*perror[:,iSim] + vAdj[iSim] )
    #end
...
return lik, s_up_stat, s_lag_stat, shock_stat, vRegime, Neff;
end
