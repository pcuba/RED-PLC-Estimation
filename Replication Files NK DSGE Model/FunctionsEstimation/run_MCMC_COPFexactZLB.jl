...
function run_MCMC_COPFexactZLB(ME_scale, nM, nSimRWMH, mhrun, nBurnRWMH,
...
# Set values in covariance matrix to zero that correspond to fixed parameters
vMask      = mPriorSpec[:,4]
vFix       = mPriorSpec[:,5]
...
vLogPosterior          = Array{Float64,2}(undef,nSimRWMH,4)
vLogPosteriorCandidate = Array{Float64,2}(undef,nSimRWMH,4)
...
(lik_i, s_up_stat_i, s_lag_stat_i, shock_stat_i, regime_stat_i, Neff) = fCOPFaugExactZLB(mY, sParam_StateSpace, mSigmaMEtilde, ME_scale, nM, pInvSigmaMEtilde);
nLogLiki = sum(lik_i)
...
for iSim = 2:nSimRWMH
    ...
    else # candidate of THETA outside of bounds or model solution did not converge
