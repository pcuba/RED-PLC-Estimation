...
function [vState,vObs,vInd,sSIM] = fSimulatePLC(sModel,sParam,vStatePrev, vEps)
...
if vEta(1) < eta1Bar(1)
   ...
   ind = 1;
    ...
    % Binding regime
   vState(:,tt) = Phi02 + Phi12*vStatePrev(1:8) + PhiEta2*vEta;

   ind = 2;
end


% Augment state with y_lag;
...
if tt==1
    ...
    delys(tt) = 100*log(sParam.gamma) + 100*(vState(5,tt) - vStatePrev(5,1) + vState(3,tt));
else
...
sSIM.z    = exp(vState(3,:));
sSIM.d    = exp(vState(4,:));
sSIM.y    = sParam.y_ss*exp(vState(5,:));
sSIM.pi   = sParam.pi_ss*exp(vState(6,:));
sSIM.c    = sParam.c_ss*exp(vState(7,:));
sSIM.R    = sParam.R_ss*exp(vState(8,:));
sSIM.BIGC = CCs;
sSIM.BIGY = YYs;
sSIM.BIGG = GGs;
sSIM.dely = delys;
