...
# -------------------------------------------------------------------------
 ...
 function fSmolyak_Elem_Isotrop(d,mu)
...
# -------------------------------------------------------------------------
   ...
   for j = 0:mu
     ...
     if (j == 0)
       ...
       #aux = zeros(m,d);           # Allocate memory to an initial auxiliary
        ...
        for id = 1:d
            ...
            if size(incr_Smol_rule,1)==0
                incr_Smol_rule=augmented
            else
            incr_Smol_rule = vcat(incr_Smol_rule, augmented);
     ...
     #---------------------------------------------------------------------
     if size(Smol_rule,1)==0
         Smol_rule=incr_Smol_rule;
     else
     Smol_rule = vcat(Smol_rule, incr_Smol_rule); # E.g., for mu=1 and d=2,
...
for i = 1:n_comb            # For each combination of subindices i1,...,id
      ...
      prev_indices = incr_indices;
          ...
          #----------------------------------------------------------------
            ...
            elseif one_comb[jd] == 2
               ...
               indices_elem_jd = collect(2^(one_comb[jd]-2)+2 : 2^(one_comb[jd]-1)+1);
           ...
           elseif (isempty(a)) # If "a" is empty, ...
               ...
               for k = 1:b_columns
                  ...
                  if isempty(z)
                      z = hcat(a,ones(a_columns,1)*b_temp);
                  else
                  z = vcat(z,hcat(a,ones(a_columns,1)*b_temp));
                  end
               end
           end
    ...
    if isempty(Smolyak_elem_iso)
        ...
        Smolyak_elem_iso = vcat(Smolyak_elem_iso,incr_indices);
