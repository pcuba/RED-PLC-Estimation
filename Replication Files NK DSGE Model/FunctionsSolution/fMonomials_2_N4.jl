...
#Translated to Julia by: Kenji Higa-Flores
#10/23/2019

function fMonomials_2_N4(vcv)

N=4
n_nodes = 2*N^2+1-2*N;    # Total number of integration nodes
...
#for i = 1:N            # In each node, random variable i takes value either                       # 1 or -1, and all other variables take value 0
...
# ----------------------------------------------------------------------
vcv_chol = cholesky(vcv);
sqrt_vcv = vcv_chol.L             # Cholesky decomposition of the variance-
                                 # covariance matrix
...
weight_nodes = [2/(N+2)*ones(size(z0,1),1);1/(N+2)^2*ones(size(z2,1),1)];
